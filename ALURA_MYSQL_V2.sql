/*Criando a base de daodos*/
create database ControleDeGastos;
create database caelum;
create database alura;
create database casadocodigo;

/*definindo qual base de dados usaremos*/
use ControleDeGastos;

/*Criando a primeira tabela chamada compras*/
...
/*Descrevendo tabela de compras*/
desc compras;

/*Inserir dados implicitamente dentro de compras*/
INSERT INTO compras VALUES(20,'2019-05-20','Lanchonete',1);
INSERT INTO compras VALUES(915.5,'2019-05-20','Guarda-roupa',0);
INSERT INTO compras VALUES(15.00,'2019-05-20','Lanchonete',0);

/*Inserindo dados defininfo Explicitamemnte dentro de compras*/
INSERT INTO compras (valor,data,observacoes,recebida) VALUES(949.99,'2019-05-20','Smartphone',0);

/* select simples */
SELECT* FROM compras;

/*Alterando a tabela comprar adicionando a coluna id*/
...
/*Adicionando auto incremento e primary key na tabela COMPRAS*/
CREATE TABLE compras(
...
insert into compras (valor, data, observacoes, recebida) values (1203.0, '2013-03-18', 'Quartos', 1);
...
SELECT * FROM compras WHERE valor>500 AND recebida =1;

/*SELECT COMPRAS MENOR QUE 500 REAIS OU MAIOR QUE 1500 REAIS*/
SELECT * FROM compras WHERE valor<500 OR valor> 1500;

/*SELECT TODAS OBSERVACOES = LANCHONETE*/
SELECT * FROM compras WHERE observacoes='Lanchonete';

/*USANDO O LIKE COM % PARA ACHAR FRASE QUE CONTENHA PARCELA%*/
SELECT * FROM compras WHERE observacoes LIKE 'Parcela%';
SELECT * FROM compras WHERE observacoes LIKE 'Higi%';
...
SELECT valor,observacoes FROM compras where data >='2012-12-15';
...
/* RESPOSTA : NÃO FUNCIONA EM BANCO DE DADOS RELACIONAL NÃO EXISTEM VALORES BOOLEANOS*/


/* 9- Selecione	todas	as	compras	com	valor	maior	que	5.000,00	ou	que	já	foram	recebidas.*/
SELECT * FROM  compras WHERE valor>5000.00 OR recebida =1;

/******************************************************************************/

/*Selecionando Valores maiores ou iguais a 1000 entre menores e iguais a 2000*/
SELECT valor,observacoes FROM compras WHERE valor>= 1000 AND valor <=2000;

/*Selecionando valores maiores ou iguais a 1000 entre menores e iguais a 2000 UTILIZANDO O BETWEEN*/
SELECT valor,observacoes FROM compras WHERE valor BETWEEN 1000 AND 2000;

SELECT * FROM compras;

/*VERIFICANDO IDS PARA UTILIZAR O UPDATE*/
SELECT valor,id FROM compras WHERE valor BETWEEN 1000 AND 2000;

/*UTILIZANDO O UPDATE*/
UPDATE compras SET valor=30000 WHERE id =1;
UPDATE compras SET observacoes ='Compra de novo AUTO' where id=1;

/*APLICANDO O UPDATE NOS IDS SELECIONADOS E APLICANDO FILTRO DE VALOR*/
UPDATE compras SET valor=0 WHERE id=3 OR id=7 OR id=22 OR id=23 OR id=37;

/*VERIFICANDO NOVAMENTE O RESULTADO DO UPDATE NOS IDS SELECIONADOS*/
SELECT valor,id FROM compras WHERE valor BETWEEN 1000 AND 2000;


/*UPDATE EM VARIAS COLUNAS AO MESMO TEMPO */
